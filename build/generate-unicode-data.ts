...
const dataDir = new URL(`../node_modules/${packageName}/`, import.meta.url);

const sourceURL = new URL(import.meta.url);
const targetURL = new URL("../src/generated/", import.meta.url);
...
const loadPropertyAliases = async (): Promise<void> => {
    const propertyAliasesPath = fileURLToPath(new URL("PropertyValueAliases.txt", sourceURL));
    let propertyAliasesContent = await readFile(propertyAliasesPath, "utf8");
...
const targetFile = new URL("UnicodeData.ts", targetURL);
...
const generateMap = async (basePath: string, alias?: string): Promise<void> => {
    ...
    const folderPath = fileURLToPath(new URL(basePath, dataDir));
...
const generateBlocksMap = async (): Promise<void> => {
    ...
    const folderURL = fileURLToPath(new URL("Block", dataDir));
