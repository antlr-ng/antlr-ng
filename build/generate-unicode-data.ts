...
const sourceURL = new URL(`../node_modules/${packageName}`, import.meta.url).toString();
...
const loadPropertyAliases = async (): Promise<void> => {
    const fn = join(dirname(fileURLToPath(import.meta.url)), "PropertyValueAliases.txt");
    let propertyAliasesContent = await readFile(fn, "utf8");
...
const targetPath = fileURLToPath(new URL("../src/generated/UnicodeData.ts", import.meta.url).toString());
...
const generateMap = async (basePath: string, alias?: string): Promise<void> => {
    ...
    for (const element of elements) {
        ...
        const rangeFileURL = new URL("ranges.js", targetURL + "/").toString();
        const set = await intervalSetFromImport(rangeFileURL);
        let counter = 0;
...
const generateBlocksMap = async (): Promise<void> => {
    ...
    const folderURL = new URL("Block/", sourceURL).toString();
    const fn = fileURLToPath(folderURL);
    const elements = await readdir(fn);
    ...
    for (let i = 0; i < elements.length; ++i) {
        ...
        const rangeFileURL = new URL("ranges.js", targetURL + "/").toString();
        const content = await import(rangeFileURL) as IDataFileContent;
