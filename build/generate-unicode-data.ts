...
const loadPropertyAliases = async (): Promise<void> => {
    let propertyAliasesContent = await readFile(pathToFileURL(join(sourcePath, "PropertyValueAliases.txt")), "utf8");
...
const writer = createWriteStream(pathToFileURL(targetPath));
